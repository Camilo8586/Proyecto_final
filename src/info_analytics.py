...
try:
    ...
    print("Hemos detectado que algunas librerías necesarias no han sido instaladas.")
    print("Utiliza el archivo instalar_dependencias.bat en la carpeta principal del proyecto")
...
class Archivo:
    ...
    def leer_csv(self):
        texto = ""
        with open(self.ruta, newline="", encoding="utf-8", errors="ignore") as f:
            lector = csv.reader(f)
            for fila in lector:
                texto += " ".join(fila) + "\n"
        self.contenido = texto
...
def resaltar(texto, palabra):
    ...
    return re.sub(
        palabra_l,
        f"\033[96m{palabra_l}\033[0m",   # 96 = cian brillante
        texto,
        flags=re.IGNORECASE
    )

def titulo(texto):
    color = "\033[94m" #94 = azul brillante
    reset = "\033[0m"
    return f"""
{color}{'-' * 40}
{texto.center(40)}
...
while True:
    try:
        ...
        if n == 0:
            ...
            if archivo_obj.tipo not in ["txt", "pdf", "docx", "csv"]:
